...
# Keep generator pinned to the latest themed template shape.
REQUIRED_TEMPLATE_MARKERS=(
  'body[data-theme="dark"]'
  'id="themeToggle"'
  "THEME_STORAGE_KEY = 'esm3-findit-theme'"
)
for marker in "${REQUIRED_TEMPLATE_MARKERS[@]}"; do
  if ! grep -Fq "$marker" "$TEMPLATE_FILE"; then
    echo "Template is not the latest themed build (missing marker: $marker)" >&2
    exit 1
  fi
done
